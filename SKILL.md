...
- **Purpose:** Persist per-account rules that map a sender (domain or address) to a Gmail **user** label; **`nina email process`** ingests inbox metadata into **`email_messages`**, applies those rules in Gmail, and optionally surfaces Telegram prompts for new high-volume senders. **`nina email infer-rules`** scans past mail only to insert new **`email_sender_rules`** from Gmail user labels (no `email_messages` writes).
