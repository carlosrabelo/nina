...
class TestBlockingInterpreter:
    ...
    def test_absolute_time_preserved(self) -> None:
        ...
        assert "[now: 2024-01-15 13:15" in call_args
    ...
    def test_duration_clamped_to_minimum(self) -> None:
        ...
        result = interpret("algo rápido agora", _llm(payload), now=datetime(2024, 1, 15, 10, 0))
    ...
    def test_skips_non_blocking_items_in_array(self) -> None:
        ...
        result = interpret("reunião de 30 minutos agora", _llm(payload), now=_NOW)
        assert len(result) == 1
        assert result[0].title == "Reunião"
