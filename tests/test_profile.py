...
class TestProfileStore:
    ...
    def test_save_then_load(self, tmp_path: Path) -> None:
        profile = Profile(mapping={
            "office": PresenceProfile(gmail=["work@co.com"], calendar=["work@co.com"]),
            "home":   PresenceProfile(gmail=["me@gmail.com"], calendar=[]),
        })
        save(profile, tmp_path)
        loaded = load(tmp_path)
        assert loaded.for_presence(PresenceStatus.OFFICE).gmail == ["work@co.com"]
...
class TestProfileInterpreter:
    ...
    def test_set_office_gmail(self) -> None:
        payload = '{"action": "update_profile", "updates": [{"presence": "office", "gmail": ["work@co.com"], "calendar": []}]}'
        result = interpret("no escritório usar work@co.com", self._llm(payload))
        assert result.action == "update_profile"
        assert result.updates[0].presence == "office"
    ...
    def test_multiple_updates(self) -> None:
        payload = '{"action": "update_profile", "updates": [{"presence": "office", "gmail": ["w@co.com"], "calendar": []}, {"presence": "home", "gmail": ["me@gmail.com"], "calendar": []}]}'
        result = interpret("office w@co.com, home me@gmail.com", self._llm(payload))
...
class TestApplyProfile:
    def test_sets_gmail(self) -> None:
        ...
        intent = ProfileIntent(action="update_profile", updates=[
            ProfileUpdate(presence="office", gmail=["work@co.com"]),
        ])
        apply(intent, profile)
        assert profile.for_presence(PresenceStatus.OFFICE).gmail == ["work@co.com"]

    def test_preserves_existing_calendar_when_not_in_update(self) -> None:
        profile = Profile(mapping={
            "office": PresenceProfile(gmail=["old@co.com"], calendar=["cal@co.com"]),
        })
        intent = ProfileIntent(action="update_profile", updates=[
            ProfileUpdate(presence="office", gmail=["new@co.com"]),
        ])
        apply(intent, profile)
        assert profile.for_presence(PresenceStatus.OFFICE).gmail == ["new@co.com"]
        assert profile.for_presence(PresenceStatus.OFFICE).calendar == ["cal@co.com"]
