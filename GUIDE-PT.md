...
## HTTP /command (MacroDroid / Externo)

Integrações externas que não suportam linguagem natural podem usar o endpoint `/command`.
...
| Comando | Descrição | Exemplo Body |
|---|---|---|
| `/presence {status}` | Definir presença | `{"command":"/presence work"}` |
| `/presence {status} note:...` | Presença com nota | `{"command":"/presence dnd note:reunião"}` |
| `/status` | Status atual | `{"command":"/status"}` |
| `/health` | Verificação de saúde | `{"command":"/health"}` |
| `/memo {texto}` | Criar memo | `{"command":"/memo ligar fornecedor"}` |
| `/activity {texto}` | Log atividade no Calendar | `{"command":"/activity reunião com cliente"}` |

**Formato da resposta:**
```json
{"ok": true, "message": "✓ work — no escritório"}
```

**Configuração MacroDroid** — Geofence → HTTP POST:
1. **Gatilho:** Zona → Entrada: "Escritório"
2. **Ação:** HTTP Request → `POST http://192.168.1.x:8765/command`
...
Registra atividades passadas no Google Calendar (fonte da verdade — os dados sobrevivem mesmo se a Nina parar).

```
/activity reunião com cliente das 14h às 15h30
/activity deploy da feature X
/activity code review PR #42 2h
```

Atividades são arredondadas para blocos de 15 minutos (início arredonda para baixo, término para cima). Se nenhum horário for informado, o sistema infere do contexto ("manhã" = 9h, "tarde" = 14h) ou usa 11:00 como padrão.

**Captura de fim de dia:** Quando você sai do trabalho (presença muda de `work` → `home`/`out`), o bot do Telegram pede para registrar as atividades do dia. Você pode responder com múltiplas atividades separadas por vírgula:

```
"reunião com cliente 9h-10h, deploy feature X 10h-12h, code review à tarde"
```

Isso cria três eventos separados no Google Calendar.
